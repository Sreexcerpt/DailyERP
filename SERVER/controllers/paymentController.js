...
//       ...paymentData,
//       createdBy: req.user?.username || 'system', // Assuming you have user info in request
...
//     // Get payment summary
//     const summary = await Payment.getPaymentSummary(recordId);
...
// // PUT /api/payment/:id - Update payment (for status changes, etc.)
...
// // DELETE /api/payment/:id - Delete payment (soft delete recommended)
...
//     const matchFilter = {};
//     if (companyId) matchFilter.companyId = companyId;
...
const getPaymentsByRecord = async (req, res) => {
  try {
    ...
    // Get payment summary
    const summary = await Payment.aggregate([
      { $match: { recordId: require('mongoose').Types.ObjectId(recordId) } },
      {
        $group: {
          _id: null,
