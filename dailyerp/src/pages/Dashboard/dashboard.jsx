...
//             params: { companyId, financialYear }
//           }),
//           fetch('http://localhost:8080/api/stock/data', {
//             params: { companyId, financialYear }
//           }),
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      setLoading(true);
      try {
        const urlParams = { companyId, financialYear };
        const [poRes, soRes, stockRes, invoiceRes, billingRes] = await Promise.all([
          fetch(buildUrl('http://localhost:8080/api/purchase-orders', urlParams)),
          fetch(buildUrl('http://localhost:8080/api/sales-orders', urlParams)),
          fetch(buildUrl('http://localhost:8080/api/stock/data', urlParams)),
          fetch(buildUrl('http://localhost:8080/api/invoiceform', urlParams)),
          fetch(buildUrl('http://localhost:8080/api/billingform', urlParams))
        ]);
  ...
  const monthlyData = useMemo(() => {
    ...
    return months;
  }, [purchaseOrders, salesOrders, materials, invoices, billings, selectedYear]);
