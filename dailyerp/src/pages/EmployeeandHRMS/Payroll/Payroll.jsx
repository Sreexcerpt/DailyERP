...
function Payroll() {
  ...
  const fetchSalaryRecords = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/api/salary-records"
      );
  ...
  const fetchEmployeeSalaryHistory = async (employeeId) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/salary-records/employee/${employeeId}`
      );
  ...
  const fetchLeaveCount = async (employeeId, month, year) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/salary-records/leave-count`,
        {
          params: {
  ...
  const generatePDF = async (
    ...
    try {
      ...
      const paidLeaveInfo = record
        ? record.paidLeaveCount || 0
        : paidLeaveCount;
      const deductibleDaysInfo = record
        ? record.deductibleDays || 0
        : deductibleDays;
      const deductionPerDayInfo = record
        ? record.deductionPerDay || 0
        : deductionPerDay;
      const totalDeductionInfo = record
      ...
      doc.text("Your Company Name", doc.internal.pageSize.width / 2, 20, {
        align: "center",
      });

      doc.setFontSize(10);
      doc.setFont(undefined, "normal");
      doc.text("Company Address Line 1", doc.internal.pageSize.width / 2, 30, {
        align: "center",
      });
      doc.text("Company Address Line 2", doc.internal.pageSize.width / 2, 35, {
        align: "center",
      });
      doc.text(
        "Phone: (123) 456-7890 | Email: company@email.com",
      ...
      doc.setFontSize(14);
      doc.setFont(undefined, "bold");
      doc.text("Salary Receipt", doc.internal.pageSize.width / 2, 55, {
        align: "center",
      });
      ...
      doc.text(`${deductionPerDayInfo.toFixed(2)}/-`, 185, 150, {
        align: "right",
      });
      ...
      doc.text(`${totalDeductionInfo.toFixed(2)}/-`, 185, 162, {
        align: "right",
      });
      ...
      doc.text(`${finalPayableSalary.toFixed(2)}/-`, 185, 174, {
        align: "right",
      });
  ...
  const filteredFaculties = faculties.filter((faculty) => {
    ...
    const matchesSearch =
      ...
      (faculty.employeeId &&
        faculty.employeeId.toLowerCase().includes(searchTerms)) ||
  ...
  const currentFaculties = filteredFaculties.slice(
    indexOfFirstFaculty,
    indexOfLastFaculty
  );
      ...
      <div className="d-flex justify-content-between align-items-center">
        <div className="content container-fluid">
                              ...
                              {selectedFaculty.profileImage ? (
                                ...
                                <div className="avatar avatar-lg bg-soft-primary">
                                  <span className="avatar-text rounded-circle">
                                    {selectedFaculty.firstName?.charAt(0)}
                                    {selectedFaculty.lastName?.charAt(0)}
                                  </span>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="col-md-3">
                          ...
                          >
                            {months
                              .filter((month) => {
                                if (
                                  salaryDetails.year === salaryDetails.joinYear
                                ) {
                                  const monthIndex = months.indexOf(month);
                                  const joinMonthIndex = months.indexOf(
                                    salaryDetails.joinMonth
                                  );
                          ...
                          >
                            {years
                              .filter(
                                (year) =>
                                  parseInt(year) >=
                                  parseInt(salaryDetails.joinYear)
                              )
                              .map((year) => (
                            ...
                            <div className="col-md-3">
                              <div className="mb-0">
                                ...
                                <input
                                  ...
                                  className="form-control"
                                  value={
                                    getDeductibleLeaveDays() * deductionPerDay
                                  }
                                  readOnly
                                />
                              </div>
                          ...
                          <div className="row align-items-center">
                            <div className="col-md-8">
                              ...
                              <small className="text-info">
                                Sundays and {paidLeaveCount} paid leave days are
                                not included in salary deductions
                              </small>
                            </div>
          ...
          <div
            ...
            tabIndex="-1"
            role="dialog"
            aria-labelledby="history-modalLabel"
            aria-hidden="true"
          >
            <div className="modal-dialog modal-xl">
              <div className="modal-content">
                <div className="modal-header bg-secondary text-white">
                  <h4 className="modal-title" id="history-modalLabel">
                    Salary History
                  </h4>
                  <button
                    type="button"
                ...
                <div className="modal-body">
                  ...
                  {employeeSalaryHistory.length > 0 ? (
                              ...
                              <tr key={record._id}>
                                ...
                                <td>
                                  ...
                                  <span className="text-muted d-block small">
                                    {record.sundayCount > 0 &&
                                      ` (Sundays: ${record.sundayCount})`}
                                  </span>
                                </td>
                                <td>
                                ...
                                <td>
                                  <strong className="text-success">
                                    ₹{record.payableSalary || record.salary}
                                  </strong>
                                </td>
                                <td>
                    ...
                    <div className="text-center py-5">
                      <div className="mb-3">
                        <i
                          className="ti ti-file-off text-muted"
                          style={{ fontSize: "48px" }}
                        ></i>
                      </div>
                      <h5>No salary records found</h5>
                      <p className="text-muted">
                        No salary has been processed for this employee yet.
                      </p>
                    </div>
                  )}
                </div>
