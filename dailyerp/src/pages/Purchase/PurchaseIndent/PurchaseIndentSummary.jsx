...
const fetchIndents = () => {
    ...
    const searchindexOfLastIndent = currentPage * searchResultsPerPage;
    const searchindexOfFirstIndent = searchindexOfLastIndent - searchResultsPerPage;
    const searchcurrentIndents = searchResults.slice(searchindexOfFirstIndent, searchindexOfLastIndent);
    const searchtotalPages = Math.ceil(searchResults.length / searchResultsPerPage);

    const searchpaginate = (pageNumber) => setCurrentPage(pageNumber);

    const SearchPaginationComponent = ({ currentPage, totalPages, onPageChange }) => {
        if (totalPages <= 1) return null;
                ...
                <div className="my-auto mb-2">
                    ...
                    <nav>
                        <ol className="breadcrumb mb-0">
                            ...
                            <li className="breadcrumb-item active" aria-current="page">Purchase Indent List</li>
                        </ol>
                    </nav>
                </div>
                                                ...
                                                {currentIndents.map((indent, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={indent._id}>
                                                            ...
                                                            <td>
                                                                {(() => {
                                                                    const date = new Date(indent.documentDate);
                                ...
                                <div>
                                    {searchcurrentIndents.length > 0 ? (
                                        <table className="table table-sm table-bordered table-hover">
                                            ...
                                            <tbody>
                                                {searchcurrentIndents.map((indent, idx) => (
                                                    <tr key={idx}>
                                                        <td>{idx + 1}</td>
                                        ...
                                        <tbody>
                                            {selectedQuotation?.items.map((item, idx) => (
                                                <tr key={item._id}>
                                                    ...
                                                    <td>{selectedQuotation.location}</td>
                                                    <td>{selectedQuotation.buyerGroup}</td>
