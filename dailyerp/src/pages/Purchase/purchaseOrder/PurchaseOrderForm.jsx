...
function PurchaseOrderForm() {
  ...
  const [quotations, setQuotations] = useState([]);
  const [enquirys, setEnquirys] = useState([]);
  const [contracts, setContracts] = useState([]); // Add contracts state
  const [selectedCategory, setSelectedCategory] = useState(null);
  const [selectedQuotationNumber, setSelectedQuotationNumber] = useState("");
  const [poNumber, setPoNumber] = useState("");
  const [date, setDate] = useState(new Date().toISOString().substring(0, 10));
  ...
  const [enquirySearchQuery, setEnquirySearchQuery] = useState("");
  const [enquirySearchResults, setEnquirySearchResults] = useState([]);
  const [contractSearchQuery, setContractSearchQuery] = useState(""); // Add contract search state
  const [contractSearchResults, setContractSearchResults] = useState([]); // Add contract search results
  ...
  useEffect(() => {
    ...
    // Add API call for contracts
    axios.get("http://localhost:8080/api/contracts/get", commonParams)
      .then((res) => {
  ...
  const handleEnquirySearch = (query) => {
    ...
    if (query.trim()) {
      const filtered = enquirys.filter((q) =>
        q.quotationNumber?.toLowerCase().includes(query.toLowerCase()) ||
        q.indentId?.toLowerCase().includes(query.toLowerCase())
      );
      setEnquirySearchResults(filtered);
    } else {
      setEnquirySearchResults([]);
  ...
  // Add Contract search handlers
  const handleContractSearch = (query) => {
    setContractSearchQuery(query);
    if (query.trim()) {
      const filtered = contracts.filter((c) =>
        c.contractNumber?.toLowerCase().includes(query.toLowerCase()) ||
        c.contractId?.toLowerCase().includes(query.toLowerCase())
      );
      setContractSearchResults(filtered);
    } else {
  ...
  const handleViewAllContracts = () => {
    setContractSearchResults(contracts);
  };
  ...
  // Add Contract selection handler
  const selectContractFromSearch = (contract) => {
    setSelectedQuotationNumber(contract.contractNumber || contract.contractId);
    handleContractChange(contract.contractNumber || contract.contractId);
    setShowQuotationModal(false);
  ...
  const closeQuotationModal = () => {
    ...
    setEnquirySearchResults([]);
    setContractSearchQuery(""); // Clear contract search
    setContractSearchResults([]); // Clear contract results
  };
  ...
  // Add Contract change handler
  const handleContractChange = (contractNumber) => {
    setSelectedQuotationNumber(contractNumber);
    const contract = contracts.find(
      (c) => c.contractNumber === contractNumber || c.contractId === contractNumber
    );
    if (contract) {
      setVendor(contract.vendorName || contract.name1);
      setDeliveryLocation(contract.items?.[0]?.location || "");
      const firstItemWithBuyerGroup = contract.items?.find(
        (item) => item.buyerGroup
      );
      if (firstItemWithBuyerGroup) {
        setBuyerGroup(firstItemWithBuyerGroup.buyerGroup);
      ...
      const mappedItems = contract.items?.map((item) => ({
        materialId: item.materialId,
        description: item.description,
  ...
  const [showPOModal, setShowPOModal] = useState(false);
  const [poGenerationType, setPOGenerationType] = useState('');
  const [externalPONumber, setExternalPONumber] = useState('');
  const [pendingSubmitData, setPendingSubmitData] = useState(null);
            ...
            <div className="card">
              <div className="card-body">
                <div className="tab-content">
                  {activeTab === "enquiry" && (
                    <div className={`tab-pane ${activeTab === "enquiry" ? `show active` : ""}`} id="bottom-tab1">
                      <div className="row mb-3">
                        <div className="col-md-6">
                          <label className="form-label">Search Enquiry Number</label>
                          <div className="input-group">
                      ...
                      <div style={{ maxHeight: "400px", overflowY: "auto" }}>
                        {enquirySearchResults.length > 0 ? (
                            ...
                            <tbody>
                              {enquirySearchResults.map((quotation, idx) => (
                                <tr key={idx}>
                                  ...
                                  <td>{new Date(quotation.documentDate).toLocaleDateString()}</td>
                                  <td>
                                    <button
                                      className="btn btn-success btn-sm"
                                      onClick={() => selectEnquiryFromSearch(quotation)}
                          ...
                          <div className="text-center py-4">
                            ...
                            <p className="text-muted">
                              {enquirySearchQuery
                                ? `No enquiries found matching "${enquirySearchQuery}"`
                                : 'Enter search term or click "View All"'}
                            </p>
                          </div>
                        )}
                  ...
                  {activeTab === "quotation" && (
                    <div className={`tab-pane ${activeTab === "quotation" ? `show active` : ""}`} id="bottom-tab2">
                      <div className="row mb-3">
                        <div className="col-md-6">
                          <label className="form-label">Search Quotation Number</label>
                          <div className="input-group">
                            ...
                            <tbody>
                              {quotationSearchResults.map((quotation, idx) => (
                                <tr key={idx}>
                                  ...
                                  <td className="text-flex">{quotation.vendorName}</td>
                  ...
                  {activeTab === "contract" && (
                    <div className={`tab-pane ${activeTab === "contract" ? `show active` : ""}`} id="bottom-tab3">
                      <div className="row mb-3">
                        <div className="col-md-6">
                          <label className="form-label">Search Contract Number</label>
                          <div className="input-group">
                            <span className="input-group-text">
                              <i className="fas fa-search"></i>
                            </span>
                      ...
                      <div style={{ maxHeight: "400px", overflowY: "auto" }}>
                        {contractSearchResults.length > 0 ? (
                              ...
                              {contractSearchResults.map((contract, idx) => (
                                <tr key={idx}>
                                  ...
                                  <td>
                                    <button
                                      className="btn btn-success btn-sm"
                                      onClick={() => selectContractFromSearch(contract)}
                                    >
                                      <i className="fas fa-check me-1"></i>Select
                                    </button>
                                  </td>
                          ...
                          <div className="text-center py-4">
                            ...
                            <p className="text-muted">
                              {contractSearchQuery
                                ? `No contracts found matching "${contractSearchQuery}"`
                                : 'Enter search term or click "View All"'}
                            </p>
                          </div>
                        )}
