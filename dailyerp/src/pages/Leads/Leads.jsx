...
const Leads = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        expectedDealValue: '',
        expectedCloseDate: '',
        campaignId: '' // Added for campaign association
    });
    ...
    const fetchLeads = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/leads?companyId=${companyId}&financialYear=${financialYear}`);
    ...
    const leadSources = ['Website', 'Social Media', 'Referral', 'Cold Call', 'Advertisement', 'Email Campaign', 'Google Ads', 'Event', 'Other'];
    const leadStatuses = ['New', 'Contacted', 'Qualified', 'Proposal', 'Negotiation', 'Closed Won', 'Closed Lost'];
    const industries = ['Technology', 'Healthcare', 'Finance', 'Manufacturing', 'Retail', 'Education', 'Other'];
    const companySizes = ['1-10', '11-50', '51-200', '201-1000', '1000+'];
            ...
            <div className='card mt-3'>
                <div className="card-header d-flex justify-content-between align-items-center">
                    <h4 className="card-title mb-0">
                        <i className="fas fa-user-plus me-2"></i>Leads Management
                    </h4>
                                    ...
                                    {filteredLeads.length > 0 ? (
                                        filteredLeads.map(lead => (
                                            <tr key={lead._id}>
                                                ...
                                                <td>
                                                    {lead.phoneNumber && (
                                                        <a href={`tel:${lead.phoneNumber}`} className="text-decoration-none">
                                                            {lead.phoneNumber}
                                                        </a>
                                                    )}
                                                </td>
                                                <td>
                                                    {lead.campaignId?.campaignName && (
                                                        <span className="badge bg-info">{lead.campaignId.campaignName}</span>
                                                    )}
                                                </td>
                                                <td>
                                                    <span className={`badge bg-${getStatusColor(lead.leadStatus)}`}>
                                                ...
                                                <td>
                                                    <span className="badge bg-secondary">{lead.leadSource}</span>
                                                </td>
                                                <td>{lead.industry}</td>
                                                <td>
                                                    <span className={`badge bg-${getScoreColor(lead.leadScore)}`}>
                                                        {lead.leadScore}/100
                                                    </span>
                                                </td>
                                                <td>
                                                    {lead.expectedDealValue ? `$${Number(lead.expectedDealValue).toLocaleString()}` : 'N/A'}
                                                </td>
                                                <td>
                                                    <button
