...
function Ledger() {
  ...
  // NEW: Unique lists for dropdowns
  const uniqueVendors = [...new Set(vendorData.map(v => v.name))];
  const uniqueCustomers = [...new Set(customerData.map(c => c.name))];

  // NEW: Filtered data based on selection
  const filteredVendorData = selectedVendor ? vendorData.filter(v => v.name === selectedVendor) : vendorData;
  const filteredCustomerData = selectedCustomer ? customerData.filter(c => c.name === selectedCustomer) : customerData;

  // NEW: Summaries for filtered data
      ...
      <div className="row">
        ...
        <div className="col-12 mb-4">
          <div className="card">
            <div className="card-header text-white d-flex justify-content-between align-items-center">
              <h5 className="mb-0">Vendor Ledger</h5>
              <small>Total Records: {filteredVendorData.length}</small>
            </div>
            <div className="card-body">
              {loading ? (
                <div className="text-center py-4">
                ...
                <>
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <span className="text-muted">
                      Page {vendorPage} of {totalVendorPages}
                      <small className="ms-2">
                        ({((vendorPage - 1) * PAGE_SIZE) + 1} - {Math.min(vendorPage * PAGE_SIZE, filteredVendorData.length)} of {filteredVendorData.length})
                      </small>
                    </span>
                    <button
                      className="btn btn-sm btn-outline-secondary"
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-header text-white d-flex justify-content-between align-items-center">
              <h5 className="mb-0">Customer Ledger</h5>
              <small>Total Records: {filteredCustomerData.length}</small>
            </div>
            <div className="card-body">
              {loading ? (
                <div className="text-center py-4">
                ...
                <>
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <span className="text-muted">
                      Page {customerPage} of {totalCustomerPages}
                      <small className="ms-2">
                        ({((customerPage - 1) * PAGE_SIZE) + 1} - {Math.min(customerPage * PAGE_SIZE, filteredCustomerData.length)} of {filteredCustomerData.length})
                      </small>
                    </span>
                    <button
                      className="btn btn-sm btn-outline-secondary"
