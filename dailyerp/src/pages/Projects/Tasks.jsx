...
const Tasks = () => {
  ...
  const loadProjectTeamMembers = async (projectId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/tasks/project/${projectId}/team-members`);
      const data = await response.json();
      setTeamMembers(data || []);
    } catch (err) {
  ...
  const handleEdit = (task) => {
    ...
    setFormData({
      ...
      description: task.description,
      projectId: task.projectId._id || task.projectId,
      assignedTo: task.assignedTo.map(member => member._id || member),
      status: task.status,
      priority: task.priority,
      dueDate: task.dueDate ? task.dueDate.split('T')[0] : '',
      estimatedHours: task.estimatedHours,
    ...
    if (task.projectId._id || task.projectId) {
      loadProjectTeamMembers(task.projectId._id || task.projectId);
    }
  ...
  const handleProjectChange = (projectId) => {
    setFormData({ 
      ...formData, 
      projectId: projectId,
      assignedTo: [] // Clear assigned members when project changes
    });
  ...
  const handleTeamMemberToggle = (memberId) => {
    ...
    if (memberIndex > -1) {
      // Remove member
      currentAssignments.splice(memberIndex, 1);
    } else {
      // Add member
      currentAssignments.push(memberId);
    }
    
    setFormData({ ...formData, assignedTo: currentAssignments });
  };
  ...
  const formatAssignedMembers = (assignedMembers) => {
    if (!assignedMembers || assignedMembers.length === 0) return 'Not Assigned';
            ...
            <div className="modal-content">
              ...
              <form onSubmit={handleSave}>
                <div className="modal-body">
                  ...
                  {formData.projectId && (
                    <div className="mb-3">
                      <label className="form-label">Assign To Team Members</label>
                      <div className="border rounded p-3" style={{maxHeight: '200px', overflowY: 'auto'}}>
                        {teamMembers.length === 0 ? (
                          <p className="text-muted mb-0">No team members found for this project</p>
                        ) : (
                          teamMembers.map((member) => (
                            <div key={member._id} className="form-check">
                              ...
                              />
                              <label className="form-check-label d-flex align-items-center" htmlFor={`member-${member._id}`}>
                                {member.profilePhoto && (
                                  <img 
                                ...
                                <div>
                                  ...
                                  <small className="text-muted">{member.department} • {member.employeeId}</small>
                                </div>
                              </label>
                            </div>
                          ))
