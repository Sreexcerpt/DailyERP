...
function POCategoryForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://localhost:8080/api/po-categories/${editId}`, formData);
        alert('PO Category updated!');
      } else {
        await axios.post('http://localhost:8080/api/po-categories', formData);
        alert('PO Category added!');
      }
      fetchCategories();
      resetForm();
    } catch (err) {
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h4 className="modal-title" id="myLargeModalLabel"> {editId ? 'Edit' : 'Create'} PO Category</h4>
