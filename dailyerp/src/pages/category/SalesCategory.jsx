...
const SalesCategory = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get('http://localhost:8080/api/salecategory');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`http://localhost:8080/api/salecategory/${editingId}`, formData);
        alert('Category updated successfully!');
      } else {
        await axios.post('http://localhost:8080/api/salecategory', formData);
            ...
            <div className="modal-dialog modal-lg">
              <div className="modal-content">
                <div className="modal-header">
                  <h4 className="modal-title" id="myLargeModalLabel"> {editingId ? 'Edit' : 'Create'} Sales Indent Category</h4>
